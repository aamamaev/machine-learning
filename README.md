...
В git выложена версия с усеченной функциональностью, позволяющая работать исключительно с категориальными признаками, заменяющая значения переменных метками групп вместо woe. Написан для python 3.6.1.
